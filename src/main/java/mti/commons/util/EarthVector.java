...
{
	static final public double X_EPSILON = 0.000000001; // degrees
	...
	public static EarthVector translateDegrees(
		...
		EarthVector result = EarthVector.fromDegrees(
			lat,
			lon);
	...
	public EarthVector(
		...
		latitude = Math.toRadians(
			point.getY());
		longitude = Math.toRadians(
			point.getX());
		elevation = point.getZ();
	...
	protected void initVector() {
		...
		final double x = Math.cos(
			latitude)
			* Math.cos(
				longitude)
			* getRadius();
		final double y = Math.cos(
			latitude)
			* Math.sin(
				longitude)
			* getRadius();
		double z;
		final double sin_lat = Math.sin(
			latitude);

		if (oblate) {
			final double ann = REKM / Math.sqrt(
				1.0 - FLAT_COEFF1 * sin_lat * sin_lat);
	...
	public Point2d getLatLon() {
		final Point2d normalizedPoint = getNormalizedPoint(
			new Point2d(
				Math.toDegrees(
					longitude),
				Math.toDegrees(
	...
	public EarthVector[] makeGreatCircleNumSegments(
		...
		if (reverseDirection) {
			final double reverseFactor = getDistanceReverseDirection(
				endpoint)
				/ getDistance(
					endpoint);
	...
	public EarthVector[] makeInterpolatedLineNumSegments(
		...
		final double baseLat = points[0].getLatitude(
			EarthVector.DEGREES);
		final double latStep = (points[segments].getLatitude(
			EarthVector.DEGREES) - baseLat) * resolution;
		final double baseLon = points[0].getLongitude(
			EarthVector.DEGREES);
		double deltaLon = (points[segments].getLongitude(
			EarthVector.DEGREES) - baseLon);
		...
		for (int i = 1; i < segments; i++) {
			...
			final double lon = get180NormalizedLon(
				baseLon + i * lonStep);
	...
	public int getNumGreatCircleSegments(
		...
		double distance;
		if (reverseDirection) {
			distance = getDistanceReverseDirection(
				endpoint);
		}
		else {
	...
	public EarthVector findPoint(
		...
		final double phi = fraction * Math.acos(
			ecfVector.dot(
				nextVector) / (ecfVector.length() * nextVector.length()));
	...
	public EarthVector findPoint(
		...
		locNorth.setLatitude(
			locNorth.getLatitude() + distR);

		// be careful! we might go over the pole
		if (locNorth.getLatitude() > RAD_90) {
			locNorth.setLatitude(
				RAD_180 - locNorth.getLatitude());
			locNorth.setLongitude(
				locNorth.getLongitude() + RAD_180);
		}
	...
	public EarthVector findPoint(
		...
		final double sinLon = Math.sin(
			lon);
		final double cosLon = Math.cos(
			lon);
		final double sinLat = Math.sin(
			lat);
		final double cosLat = Math.cos(
			lat);
	...
	public double getVectorDistanceKMSq(
		final EarthVector loc ) {
		final Vector3d delta = getVector(
			loc);
	...
	public double getDistance(
		final EarthVector loc ) {
		double dist = getEarthRadiusKM() * (Math.acos(
			ecfVector.dot(
				loc.getVector()) / (ecfVector.length() * loc.getVector().length())));
	...
	public double getDistanceReverseDirection(
		final EarthVector loc ) {
		double dist = getEarthRadiusKM() * (2 * Math.PI - Math.acos(
			ecfVector.dot(
				loc.getVector()) / (ecfVector.length() * loc.getVector().length())));
	...
	public double getSphereDistance(
		final EarthVector loc ) {
		return (getEarthRadiusKM() * (Math.acos(
			Math.sin(
				latitude)
				* Math.sin(
					loc.getLatitude())
	...
	public double getAzimuth(
		...
		final EarthVector thisNorm = new EarthVector(
			this);
		thisNorm.setElevation(
			0);
		final EarthVector otherNorm = new EarthVector(
			loc);
		otherNorm.setElevation(
			0);
		return thisNorm.internalGetAzimuth(
			otherNorm);
	}
	...
	static public double get180NormalizedLon(
		final double lon ) {
		double newLon = lon;
		while (newLon < -180) {
			newLon += 360;
	...
	static public Point2d getNormalizedPoint(
		final Point2d pt ) {
		return new Point2d(
			get180NormalizedLon(
				pt.x),
